...
(function(window, undefined) {
	...
	_extend(KDialog, KWidget, {
		init: function(options) {
			...
			self.closeIcon = K('<span class="ke-dialog-icon-close" title="' + closeBtn.name + '"></span>').click(
				closeBtn.click
			)
			...
			_each(
				[{ btn: previewBtn, name: 'preview' }, { btn: yesBtn, name: 'yes' }, { btn: noBtn, name: 'no' }],
				function() {
					if (this.btn) {
		...
		create: function() {
			...
			K.each(self.items, function(i, name) {
				if (name == '|') {
					...
					htmlList.push(
						'<span class="ke-toolbar-icon ke-toolbar-icon-url ke-icon-' + name + '" unselectable="on"></span></span>'
					)
				}
			})
	...
	_plugin('core', function(K) {
		...
		self.beforeGetHtml(function(html) {
			...
			return html
				.replace(/(<(?:noscript|noscript\s[^>]*)>)([\s\S]*?)(<\/noscript>)/gi, function($0, $1, $2, $3) {
			...
			.change(function() {
				...
				r.onload = function() {
					$.post('/files/', { name: file.name, file: r.result.replace(/data:.*;base64,/i, '') }, function(data) {
						self.insertHtml('<img src="' + data.url + '"/>')
					})
				}
...
KindEditor.plugin('insertfile', function(K) {
	...
	self.clickToolbar(name, function() {
		self.plugin.fileDialog({
			clickFn: function(url, title) {
				var html =
					'<a class="ke-insertfile" href="' + url + '" data-ke-src="' + url + '" target="_blank">' + title + '</a>'
				self
					.insertHtml(html)
					.hideDialog()
					.focus()
			...
			for (var name in postParams) {
				if (postParams.hasOwnProperty(name)) {
					paramStringPairs.push(
						encodeURIComponent(name.toString()) + '=' + encodeURIComponent(postParams[name].toString())
					)
				}
			}
		}
